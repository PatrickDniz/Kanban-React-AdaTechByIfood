...
const Kanban: React.FC<KanbanProps> = ({ tasks }) => {
  ...
  const moveTaskToNextColumn = (taskId: string, currentCol: string) => {
    let taskToMoveToDoing
    let taskToMoveToDone
    switch (currentCol) {
      case 'A Fazer':
        taskToMoveToDoing = todo.find((task) => task.key === taskId)
        ...
        break
      case 'Em Progresso':
        taskToMoveToDone = inProgress.find((task) => task.key === taskId)
        if (taskToMoveToDone) {
  ...
  const moveTaskToPreviousColumn = (taskId: string, currentCol: string) => {
    let taskToMoveToToDoing
    let taskToMoveToInProgress
    switch (currentCol) {
      case 'Em Progresso':
        taskToMoveToToDoing = inProgress.find((task) => task.key === taskId)
        ...
        break
      case 'Concluído':
        taskToMoveToInProgress = done.find((task) => task.key === taskId)
